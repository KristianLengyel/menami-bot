...
async def _render_attachment(bot, card):
    ...
    dye = await bot.db.get_card_dye(card["card_uid"])
    apply_glow = dye is not None
    glow_color = dye[2] if dye else None
    glow_thickness = dye[4] if dye and len(dye) >= 5 else None
...
def _firebase_bucket():
    global _FBASE_INIT
    if not _FBASE_INIT:
        key_path = os.getenv("FIREBASE_KEY", "").strip()
        bucket_name = os.getenv("FIREBASE_BUCKET", "").strip()
        if not key_path or not bucket_name:
            raise RuntimeError("Set FIREBASE_KEY (path to service account JSON) and FIREBASE_BUCKET env vars.")
        try:
            get_app()
        except ValueError:
...
async def _upload_bytes_to_firebase(data: bytes, dest_path: str, content_type: str = "image/png") -> tuple[str, int, str]:
    bkt = _firebase_bucket()
    ...
    try:
        ...
        url = f"https://storage.googleapis.com/{bkt.name}/{dest_path}"
    return url, len(data), content_type
...
class InventoryCog(commands.Cog):
    ...
    async def cmd_imgset(self, ctx, edition: int, *, query: str):
        ...
        if not ctx.message.attachments:
            return await ctx.send("Please attach an image (≈520×700px, ≤200KB).")

        a = ctx.message.attachments[0]
        try:
            data = await a.read()
        except Exception:
            return await ctx.send("Couldn't read the attachment. Try again.")

        if len(data) > 200_000:
            return await ctx.send("⚠️ Keep images ≤ 200KB.")

        mime = a.content_type or "image/png"

        sslug = _slugify(series)
        cslug = _slugify(character)
        dest_path = f"{sslug}/{cslug}/ed{int(edition)}.png"

        try:
            url, size, eff_mime = await _upload_bytes_to_firebase(data, dest_path, mime)
        except Exception as e:
            return await ctx.send(f"Upload failed: `{e}`")

        try:
            await self.bot.db.set_character_image(series, character, int(edition), url, size, eff_mime)
        except Exception as e:
            return await ctx.send(f"Saved to storage, but DB update failed: `{e}`")

        await ctx.send(f"✅ Image uploaded & set for **{character}** · **{series}** · ◈{edition}\n{url}")
